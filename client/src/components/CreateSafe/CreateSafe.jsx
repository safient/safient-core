...
function CreateSafe({ network, address, writeContracts }) {
  ...
  const onFormSubmit = async (e) => {
    ...
    if (inheritorAddress !== '' && inheritorAddress.length === 42) {
      ...
      try {
        const tx = await writeContracts.SafexMain.createSafe(inheritorAddress, metaevidenceURI, { value: totalPrice });
  ...
  return (
    <>
      <Note label='Note '>
        ...
        stored in the safe. It can be used by the inheritor to create a claim. Owner of the safe can recover funds in
        the safe at anytime.
      </Note>
      <Spacer />
      <Link target='_blank' href={safexAgreementLink} style={{ display: 'flex', alignItems: 'flex-start' }}>
        <Text b>Safe Creation Agreement</Text>

...
function CreateClaim({ network, writeContracts }) {
  ...
  const onFormSubmit = async (e) => {
    e.preventDefault();
    if (safeId !== '' && safeId !== null && Number(safeId) !== 0 && Number(safeId) > 0) {
      if (buffer !== null) {
        if (evidenceName !== '' && evidenceDescription !== '') {
          ...
          try {
            const tx = await writeContracts.SafexMain.createClaim(Number(safeId), evidenceURI);
  ...
  return (
    <>
      ...
      <form>
        <Row>
          <Col>
            <Input status='secondary' type='number' onChange={(e) => setSafeId(e.target.value)} width='75%'>
              <Text b>Safe Id :</Text>
            </Input>
          </Col>
